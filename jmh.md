...
* In [maven](https://search.maven.org/classic/#search%7Cga%7C1%7Corg.openjdk.jmh) look for 
`jmh-core` and `jmh-generator-annprocess`.

## Types of Benchmarks
* `@BenchmarkMode` or `-bm`
  - All: all the benchmark modes.
...
* `@Fork` or `-f`: How many real run of benchmark,
* `@Warmup` or `-wi`: How many warmup runs of benchmark.

## State
* `@State` 
  - encapsulate the state on which benchmark is working on.
  - defines to which extent it is shared among the worker threads.
  - usually injected into Benchmark methods as arguments.
  - JMH takes care of their instantiation and sharing.
* `@Scope`
  - `Benchmark`: all instances of the same type will be shared across all
    worker threads. Setup and TearDown methods on this state object would
    be performed by one of the worker threads, and only once per Level. No
    other threads would ever touch the state object.
  - `Group`: all instances of the same type will be shared across all threads within
    the same group. Each thread group will be supplied with its own state
    object. Setup and TearDown methods on this state object would be performed
    by one of the group threads, and only once per Level. No other threads
    would ever touch the state object.
  - `Thread`: With thread scope, all instances of the same type are distinct,
    even if multiple state objects are injected in the same benchmark. Setup
    and TearDown methods on this state object would be performed by single
    worker thread exclusively, and only once per Level. No other threads
    would ever touch the state object.
